...
| **M4 – CRUD Operations** | **Needs API design** | Add / edit / delete a job | 5 | • Define persistence strategy for mutable jobs<br>• Add POST/PUT/DELETE endpoints in backend API layer<br>• Add API tests for CRUD behavior<br>• Add UI for adding/editing/deleting jobs<br>• Add corresponding unit & UI tests<br>• Update CI to include new tests | CRUD API contract is documented; backend endpoints persist changes safely; all CRUD actions work on simulator; CI shows **✅** for all new tests |
