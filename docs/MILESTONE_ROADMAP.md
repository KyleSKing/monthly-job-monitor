...
| **M4 – CRUD Operations** | **Backend done; iOS pending** | Add / edit / delete a job | 5 | • ✅ Define persistence strategy (file store behind swappable interface)<br>• ✅ Add POST/PUT/DELETE endpoints in backend API layer<br>• ✅ Add API tests for CRUD behavior<br>• Add iOS `APIService` create/update/delete methods<br>• Add UI for adding/editing/deleting jobs<br>• Add corresponding unit & UI tests | Backend CRUD contract documented and endpoints merged to `develop` with CI ✅; remaining: iOS `APIService` methods, UI, and simulator verification |
